...
Overlay" simple labeling to:
    ...
    - ...which activates the callout (grey dashed leader,
      anchorPoint=pole_of_inaccessibility - inside the ring band for an
      annulus). The authored callout was curved; it is converted to the
      straight "simple" type so leaders match the FieldNotebook comment
      callouts (inject_dynamic_callouts.py), whose dynamics are mirrored
      too: dist=30 / maximumDistance=150 map units (1x / 5x the scale-5000
      label offset unit), minLength 1 MM so no stub is drawn on
      inside-placed labels.
    - overlapHandling=AllowOverlapIfRequired: zone labels never vanish.

Overlay labeling is NOT regenerated by script_setmapping (only FieldNotebook
is), so this edit persists; the map-unit distances are baked for the
template's symbologyReferenceScale of 5000.
...
U = 30.0  # offset_for_scale(5000) = 0.006 * 5000 map units, mirrors FieldNotebook
PLACEMENT_ATTRS = {
...
def main():
    ...
    assert opts.get("anchorPoint") == "pole_of_inaccessibility", opts.get("anchorPoint")
    for k in CURVED_ONLY_OPTS:
